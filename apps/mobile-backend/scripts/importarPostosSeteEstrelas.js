/**
 * Importa os postos "Sete Estrelas" (dados reais de localização, já
 * geocodificados) pro banco de fluxo. Roda uma vez, manual:
 *
 *   node scripts/importarPostosSeteEstrelas.js
 *
 * Fonte dos dados: apps/mobile-front/sete_estrelas_coords.js — copiados aqui
 * como valor estático porque o mobile-front usa ES module (`export const`) e
 * o mobile-backend é CommonJS; mais simples duplicar os dados de localização
 * (que não mudam) do que fazer interoperar dois sistemas de módulo diferentes
 * só pra isso.
 *
 * ATENÇÃO: essa fonte não tem preço nenhum, só nome/endereço/lat/lng. Os
 * preços gerados abaixo são PLACEHOLDER — variação aleatória em cima de um
 * valor base, só pra ter dado pra testar a tela de comparação de preço no
 * simulador. Não são preços reais. Trocar quando cada posto for credenciado
 * de verdade (e nesse momento, também trocar a api_key_hash placeholder por
 * uma de verdade, gerada no credenciamento do agente daquele posto).
 */

const bcrypt = require('bcrypt');
const { knexFluxo, inicializarBancoFluxo } = require('../src/db/dbFluxo');
...
function precoPlaceholder(base) {
  const variacao = (Math.random() - 0.5) * 0.4; // +/- R$0,20
  return Math.round((base + variacao) * 100) / 100;
}

async function importar() {
  await inicializarBancoFluxo();

  const produtos = await knexFluxo('produtos').select('id', 'descricao');
  if (produtos.length === 0) {
    throw new Error('Tabela produtos vazia — rode o servidor uma vez antes (o seed de dev cria o catálogo).');
  }
  ...
  for (const p of POSTOS) {
    const existe = await knexFluxo('postos').where({ codigo_local: p.id }).first();
    if (existe) { ignorados++; continue; }

    // Placeholder — este posto ainda não tem agente/credenciamento real.
    // Trocar por uma api_key de verdade quando o posto for de fato conectado.
    const apiKeyHash = await bcrypt.hash('placeholder-' + p.id, 10);
    const endereco = p.bairro ? `${p.endereco} - ${p.bairro}` : p.endereco;
    ...
    for (const produto of produtos) {
      const base = PRECOS_BASE[produto.descricao] ?? 5.5;
      await knexFluxo('postos_produtos').insert({
        posto_id: postoId,
        produto_id: produto.id,
        preco: precoPlaceholder(base),
      });
    }
  ...
  console.log(`Importação concluída: ${criados} postos criados, ${ignorados} já existiam.`);
  console.log('ATENÇÃO: os preços gerados são fictícios (placeholder pra teste), não reais.');
  await knexFluxo.destroy();
}
